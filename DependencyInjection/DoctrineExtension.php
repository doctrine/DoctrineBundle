...
class DoctrineExtension extends AbstractDoctrineExtension
{
    ...
    {
        // configuration
        $configuration = $container->setDefinition(sprintf('doctrine.dbal.%s_connection.configuration', $name), $this->createChildDefinition('doctrine.dbal.connection.configuration'));
        $logger = null;
        ...
        if ($connection['profiling']) {
            ...
            $container->setDefinition($profilingLoggerId, $this->createChildDefinition('doctrine.dbal.logger.profiling'));
            $profilingLogger = new Reference($profilingLoggerId);
            $container->getDefinition('data_collector.doctrine')->addMethodCall('addLogger', array($name, $profilingLogger));

            if (null !== $logger) {
                $chainLogger = $this->createChildDefinition('doctrine.dbal.logger.chain');
        ...
        // event manager
        $container->setDefinition(sprintf('doctrine.dbal.%s_connection.event_manager', $name), $this->createChildDefinition('doctrine.dbal.connection.event_manager'));
        ...
        $def = $container
            ->setDefinition(sprintf('doctrine.dbal.%s_connection', $name), $this->createChildDefinition('doctrine.dbal.connection'))
            ->setArguments(array(
                $options,
    ...
    {
        $ormConfigDef = $container->setDefinition(sprintf('doctrine.orm.%s_configuration', $entityManager['name']), $this->createChildDefinition('doctrine.orm.configuration'));
        ...
        $container
            ->setDefinition($managerConfiguratorName, $this->createChildDefinition('doctrine.orm.manager_configurator.abstract'))
        ...
        $container
            ->setDefinition(sprintf('doctrine.orm.%s_entity_manager', $entityManager['name']), $this->createChildDefinition('doctrine.orm.entity_manager.abstract'))
            ->setArguments(array(
    ...
    /**
     * @param $parent
     * @return Symfony\Component\DependencyInjection\DefinitionDecorator|ChildDefinition
     */
    private function createChildDefinition($parent)
    {
        if (class_exists('Symfony\Component\DependencyInjection\DefinitionDecorator')) {
            return new Symfony\Component\DependencyInjection\DefinitionDecorator($parent);
        }
